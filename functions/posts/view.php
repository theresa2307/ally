...
<body>
	<div class="content_box">
		...
		$zeile2 = $query2->fetchAll( PDO::FETCH_ASSOC ); //ergebnisse aus $query2 werden in einem multidimensionalen array gespeichert
		while ( $zeile = $query->fetchObject() ) { //alles was in der db steht, alle post werden ausgelesen, was oben gesucht wird bei query
			...
			for ( $i = 0; $i < count( $zeile2 ); $i++ ) { //geht alle nested arrays durch
					if (($zeile2[$i]['username'] == $zeile->username) and ( strlen( $zeile2[$i]['datei'] ) > 1 )){ //überprüft ob es ein nested array gibt in dem der username mit dem postautor übereinstimmt und ein bild hinterlegt wurde
						echo "<a class='post_header_userpic' href='?page=profile&user=$zeile->username'><img style='width:50px; height:50px; border-radius:100%;' src='./functions/profil/uploads/files/".$zeile2[$i]['datei']."'/></a>"; //gibt das entsprechende bild aus
					}
				}
